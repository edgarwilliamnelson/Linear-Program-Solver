...
private static final double ZERO_THRESHOLD = Math.pow(10, (-10));
...
public static void main (String[] args) {
        ...
        String val = Double.toString(values.get("zeta").constant);
        if(val.length() > 9){
        ...
        for(int i = 0; i < dictionary.get(0).size(); i++){
          bd = new BigDecimal(values.get("x" + (i+1)).constant);
          bd = bd.round(new MathContext(7));
          s = bd.toString();
          s = s.indexOf(".") < 0 ? s : s.replaceAll("0*$", "").replaceAll("\\.$", "");
          xs = xs + s + " ";
...
//Method to generate and solve the auxiliary problem for an initally infeasible LP.
private static void solve_auxiliary(){
    ...
    //If the auxiliary is unbounded, omega must be non zero and the original is infeasible
    if(unbounded){
      unbounded = false;
      infeasible = true;
    	return;
    ...
    //If the zeta value is non zero, the original LP is infeasible
    if(!zero(values.get("zeta").constant)){
    	infeasible = true;
    	return;
    }
    
    //Pivot omega into into the objective function to be removed.
    ...
    //First set the objective row to all zeros
    values.remove("zeta");
    ...
    //If a variable in the original objective row is currently basic: Add ((it's row equation) * (it's coefficient in the original objective row)) to the new objective row.
    for(int i = 0; i < original_objective_row.size(); i++){
        String variable = original_objective_indicies.get(i);
...
private static boolean zero(double d){
  if(Double.compare(Math.abs(d), ZERO_THRESHOLD) < 0)
    return true;
  return false;
}
...
private static void print_dict(){
      for(int i = 0; i < dictionary.size(); i++){
      String line = "";
      String row = index_to_variable_map.get(i);
      line = line + row + ": " + values.get(row).toString();
      for(int j = 0; j < dictionary.get(i).size(); j++){
        line = line + " " + dictionary.get(i).get(j) + objective_variable_indicies.get(j) + " ";
      }
      System.out.println(line);
    }
}

//Value class for lexicographic method. The value of Zeta and each variable is maintained by an instance of this class.
//Each class contains the real number value of the row as a Double, then an array of epsilons. During a comparison if the doubles match the epsilons are compared lexicographicly.
private static class Value {
  ...
  public void max_epsilons(){
    for(int i = 0; i < epsilons.length; i++){
      epsilons[i] = Double.MAX_VALUE;
    }
  }
